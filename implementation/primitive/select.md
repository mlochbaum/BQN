...
Before getting to SIMD, one general way to reduce overhead if `inds` is small is to convert to a single selection, `(inds+⌜˜↕⊸×´≢arr)⊏⥊arr` for rank-2 `arr` with adjustments possible for other ranks. This is effectively a specialization of the axis merging
...
However, arbitrarily large result rows can be handled by constructing the result in columns, where each corresponds to a manageable slice of `𝕨`. Working column-wise
