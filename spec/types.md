...
Of the types specified, the first three are considered *data types* and the next three *operation types*. We first describe the operation types and the namespace; the remainder of this page will be dedicated to the data types. A member of any operation type accepts some number of *inputs* and returns a *result*; inputs and the result are values of any type. An evaluation begins when the operation is given inputs (*called*), and concludes when it returns or causes an error. It may involve side effects such as manipulating variables and calling other operations within its scope, or performing I/O.
